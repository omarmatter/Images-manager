...
<script >
import {defineComponent, computed, onMounted, watch, nextTick, inject, ref, provide, onBeforeMount} from "vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        const routeCreateName = computed(() => {
            console.log(store.getters.routeCreateName)
            return store.getters.routeCreateName;
        });
        const routeCreate = computed(() => {
            console.log(store.getters.routeCreate)
            return store.getters.routeCreate;
        });
    ...
    //     setTimeout(function (){
    //         next(vm => {
    //             vm.$store.dispatch(Actions.UPDATE_LANGUAGES , ['ar' , 'en' , 'fr']);
    //         })
    //
    //     } , 500);
    // }
